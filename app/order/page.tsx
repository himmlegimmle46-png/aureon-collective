...
// Fixes clipping: min-height + leading + a touch more vertical padding
const inputCls =
  "w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white " +
  "leading-relaxed min-h-[42px] " +
  "outline-none focus:border-yellow-400/25 focus:shadow-[0_0_24px_rgba(255,204,77,0.10)] " +
  "disabled:opacity-70 disabled:cursor-not-allowed placeholder:text-white/35";
...
export default function OrderPage() {
  ...
  // force rerender so cooldown text updates (no eslint "tick unused")
  const [, forceRerender] = useState(0);
  useEffect(() => {
    const t = setInterval(() => forceRerender((x) => x + 1), 250);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl">
      <div className="flex flex-col gap-6">
        ...
        {/* cleaner banner (less glow) */}
        <div className="rounded-2xl border border-white/10 bg-black/30 p-4 text-sm">
          <div className="font-semibold text-yellow-200">Before purchase</div>
          <div className="mt-1 text-white/70">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="ml-2 text-yellow-200 underline decoration-yellow-300/40 hover:text-yellow-100"
            >
